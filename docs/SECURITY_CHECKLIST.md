...
| **Vulnerability reporting address and response process** | **PARTIAL** | address chosen: security@mineai.dev. A DNS check on 2026-09-21 found no MX records for mineai.dev, so the mailbox cannot receive mail yet. The response and incident process is written and its best-effort targets adopted (`VULNERABILITY_RESPONSE.md`). Still open: the mailbox must receive mail before any public launch |
